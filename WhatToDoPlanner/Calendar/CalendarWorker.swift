...
protocol CalendarWorkerProtocol {
...
final class CalendarWorker: CalendarWorkerProtocol {
        ...
        if let tokenData = keychainService.getData(forKey: "userToken"),
           let token = String(data: tokenData, encoding: .utf8) {
            ...
            guard let url = URL(string: urlText) else {
                print("Incorrect URL")
                completion(.failure(NSError(domain: "CalendarError", code: 1, userInfo: [NSLocalizedDescriptionKey: "Invalid URL"])))
                return
            }
